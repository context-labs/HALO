...
agent / LLM / tool span emitted by the SDK will be appended to a plain JSONL
file in the inference.net OTLP-shaped export format that the HALO Engine reads
directly (no gunzip step).
...
Usage:
    ...
    processor.shutdown()  # flush the file handle
...
class InferenceOtlpFileProcessor(TracingProcessor):
    ...
    07-export.md. The HALO Engine reads this file directly. Safe to use
    in dev / evals / one-off exports. For production ingest you'd replace
    the file sink with an HTTP POST to your edge proxy — the
    `span_to_otlp_line` call is the same."""
...
def setup_tracing(
    ...
    so the file handle flushes its final lines.
    """
