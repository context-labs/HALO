...
async def stream_engine_async(
    ...
    *,
    ...
    tool / agent activity. Routing: if ``CATALYST_OTLP_TOKEN`` or
    ``INFERENCE_API_KEY`` is set, spans are uploaded to inference.net over
            ...
            with halo_agent_span(span_name="halo-root.run", agent_id="halo", system="openai"):
                ...
                async def _drive() -> None:
                    agent_runner = OpenAiAgentRunner(
                        ...
                        emit_run_checkpoints=engine_config.emit_run_checkpoints,
                    )
                    try:
