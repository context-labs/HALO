...
async def _build_run_state(cfg) -> EngineRunState:
    ...
    index_path = await TraceIndexBuilder.ensure_index_exists(
        trace_path=trace_path, config=cfg.trace_index,
    )
...
async def probe_depth_zero_no_subagent_tool() -> None:
    ...
    root_exec = AgentExecution(
        agent_id="root-x", agent_name="root", depth=0,
        parent_agent_id=None, parent_tool_call_id=None,
    )
    sdk_agent = build_root_sdk_agent(
        engine_config=cfg, run_state=run_state, agent_execution=root_exec,
    )
    ...
    _check(len(names) > 0,
           "depth=0: root still has leaf tools",
           observed=f"tools={names}")
...
async def probe_depth_one_root_has_subagent_tool() -> None:
    ...
    names = _tool_names(sdk_agent.tools)
    _check("call_subagent" in names,
           "depth=1: root has call_subagent tool",
           observed=f"tools={names}")
...
async def probe_depth_two_intermediate_has_subagent_tool() -> None:
    ...
    _check("call_subagent" not in d2,
