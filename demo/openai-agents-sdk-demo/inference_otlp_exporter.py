"""
Convert openai-agents-sdk spans into inference.net's OTLP-shaped JSONL
line format (see 07-export.md in the inference-net monorepo).

Vendored from the monorepo reference:
  inference/.ai-docs/design-docs/collecting-customer-traces/
    openai-agents-sdk-span-conversion.md
  (commit 8d767e970e on branch abe/traces-viewer).

Usage:

    from agents import Agent, Runner, add_trace_processor
    from inference_otlp_exporter import (
        ExportContext,
        InferenceOtlpFileProcessor,
    )

    ctx = ExportContext(project_id="prj_abc123", service_name="support-bot")
    add_trace_processor(InferenceOtlpFileProcessor("traces.jsonl.gz", ctx=ctx))

    agent = Agent(name="assistant", instructions="Be helpful.")
    Runner.run_sync(agent, "Hello")

The processor is additive — the default OpenAI processor still runs. Use
`set_trace_processors([...])` instead of `add_trace_processor(...)` if you
